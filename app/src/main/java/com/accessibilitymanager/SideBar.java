...
public class SideBar extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            // 每行高度固定为 14dp
            float singleHeight = getResources().getDisplayMetrics().density * 14;
            height = (int) (singleHeight * b.length);
        ...
        switch (action) {
            ...
            default:
                if (oldChoose != c) {
                    if (c >= 0 && c < b.length) {
                        if (listener != null) {
                            float touchY = (c + 0.5f) * (getHeight() / (float)b.length);
    ...
    public interface OnTouchingLetterChangedListener {
        void onTouchingLetterChanged(String s, float touchY);
        void onTouchEnd();
    }
}
